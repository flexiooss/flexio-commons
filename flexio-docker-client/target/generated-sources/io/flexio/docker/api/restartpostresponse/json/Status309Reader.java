...
import io.flexio.docker.api.restartpostresponse.Status309.Builder;
...
public class Status309Reader {
  public Status309 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status309.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public Status309[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status309> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status309[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.restartpostresponse.Status309 array, current token was %s", parser.currentToken()));
  }
