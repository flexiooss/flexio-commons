...
import io.flexio.docker.api.ContainerDeleteResponse.Builder;
import io.flexio.docker.api.containerdeleteresponse.json.Status204Reader;
import io.flexio.docker.api.containerdeleteresponse.json.Status400Reader;
...
public class ContainerDeleteResponseReader {
  public ContainerDeleteResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerDeleteResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public ContainerDeleteResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerDeleteResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerDeleteResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.ContainerDeleteResponse array, current token was %s", parser.currentToken()));
  }
