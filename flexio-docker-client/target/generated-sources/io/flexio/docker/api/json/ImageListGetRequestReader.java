...
import io.flexio.docker.api.ImageListGetRequest.Builder;
...
public class ImageListGetRequestReader {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ImageListGetRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          ...
          case DIGESTS: {
            Set<JsonToken> expectedTokens = DIGESTS_EXPECTEDTOKENS;
            builder.digests(this.readValue(parser, jsonParser -> jsonParser.getBooleanValue(), "digests", expectedTokens));
            break;
          }
          default: {
  ...
  public ImageListGetRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ImageListGetRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ImageListGetRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.ImageListGetRequest array, current token was %s", parser.currentToken()));
  }
