...
import io.flexio.docker.api.KillPostResponse.Builder;
import io.flexio.docker.api.killpostresponse.json.Status204Reader;
import io.flexio.docker.api.killpostresponse.json.Status309Reader;
import io.flexio.docker.api.killpostresponse.json.Status404Reader;
import io.flexio.docker.api.killpostresponse.json.Status500Reader;
...
public class KillPostResponseReader {
  public KillPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          KillPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    KillPostResponse.Builder builder = KillPostResponse.builder();
  ...
  public KillPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<KillPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new KillPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.KillPostResponse array, current token was %s", parser.currentToken()));
  }
