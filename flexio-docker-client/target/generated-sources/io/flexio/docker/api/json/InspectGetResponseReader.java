...
import io.flexio.docker.api.InspectGetResponse.Builder;
import io.flexio.docker.api.inspectgetresponse.json.Status200Reader;
import io.flexio.docker.api.inspectgetresponse.json.Status404Reader;
import io.flexio.docker.api.inspectgetresponse.json.Status500Reader;
...
public class InspectGetResponseReader {
  public InspectGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          InspectGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    InspectGetResponse.Builder builder = InspectGetResponse.builder();
  ...
  public InspectGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<InspectGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new InspectGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.InspectGetResponse array, current token was %s", parser.currentToken()));
  }
