...
import io.flexio.docker.api.CreateImagePostRequest.Builder;
...
public class CreateImagePostRequestReader {
  private static Set FROMIMAGE_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set REPO_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set TAG_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public CreateImagePostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          CreateImagePostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case FROMIMAGE: {
            Set<JsonToken> expectedTokens = FROMIMAGE_EXPECTEDTOKENS;
            builder.fromImage(this.readValue(parser, jsonParser -> jsonParser.getText(), "fromImage", expectedTokens));
            break;
          }
          case REPO: {
            Set<JsonToken> expectedTokens = REPO_EXPECTEDTOKENS;
            builder.repo(this.readValue(parser, jsonParser -> jsonParser.getText(), "repo", expectedTokens));
            break;
          }
          case TAG: {
            Set<JsonToken> expectedTokens = TAG_EXPECTEDTOKENS;
            builder.tag(this.readValue(parser, jsonParser -> jsonParser.getText(), "tag", expectedTokens));
            break;
          }
          default: {
  ...
  public CreateImagePostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<CreateImagePostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new CreateImagePostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.CreateImagePostRequest array, current token was %s", parser.currentToken()));
  }
