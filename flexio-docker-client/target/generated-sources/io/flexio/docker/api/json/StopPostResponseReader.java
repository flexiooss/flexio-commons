...
import io.flexio.docker.api.StopPostResponse.Builder;
import io.flexio.docker.api.stoppostresponse.json.Status204Reader;
import io.flexio.docker.api.stoppostresponse.json.Status309Reader;
import io.flexio.docker.api.stoppostresponse.json.Status404Reader;
import io.flexio.docker.api.stoppostresponse.json.Status500Reader;
...
public class StopPostResponseReader {
  public StopPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          StopPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    StopPostResponse.Builder builder = StopPostResponse.builder();
  ...
  public StopPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<StopPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new StopPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.StopPostResponse array, current token was %s", parser.currentToken()));
  }
