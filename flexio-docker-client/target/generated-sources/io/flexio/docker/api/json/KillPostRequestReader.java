...
import io.flexio.docker.api.KillPostRequest.Builder;
...
public class KillPostRequestReader {
  private static Set SIGNAL_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set CONTAINERID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public KillPostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          KillPostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case SIGNAL: {
            Set<JsonToken> expectedTokens = SIGNAL_EXPECTEDTOKENS;
            builder.signal(this.readValue(parser, jsonParser -> jsonParser.getText(), "signal", expectedTokens));
            break;
          }
          case CONTAINERID: {
  ...
  public KillPostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<KillPostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new KillPostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.KillPostRequest array, current token was %s", parser.currentToken()));
  }
