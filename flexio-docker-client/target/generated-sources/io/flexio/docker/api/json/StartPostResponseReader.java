...
import io.flexio.docker.api.StartPostResponse.Builder;
import io.flexio.docker.api.startpostresponse.json.Status204Reader;
import io.flexio.docker.api.startpostresponse.json.Status309Reader;
import io.flexio.docker.api.startpostresponse.json.Status404Reader;
import io.flexio.docker.api.startpostresponse.json.Status500Reader;
...
public class StartPostResponseReader {
  public StartPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          StartPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    StartPostResponse.Builder builder = StartPostResponse.builder();
  ...
  public StartPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<StartPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new StartPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.StartPostResponse array, current token was %s", parser.currentToken()));
  }
