...
import io.flexio.docker.api.CreateImagePostResponse.Builder;
import io.flexio.docker.api.createimagepostresponse.json.Status200Reader;
import io.flexio.docker.api.createimagepostresponse.json.Status404Reader;
import io.flexio.docker.api.createimagepostresponse.json.Status500Reader;
...
public class CreateImagePostResponseReader {
  public CreateImagePostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          CreateImagePostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    CreateImagePostResponse.Builder builder = CreateImagePostResponse.builder();
  ...
  public CreateImagePostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<CreateImagePostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new CreateImagePostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.CreateImagePostResponse array, current token was %s", parser.currentToken()));
  }
