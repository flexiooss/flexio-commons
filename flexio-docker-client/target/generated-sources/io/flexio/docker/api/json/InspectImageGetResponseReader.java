...
import io.flexio.docker.api.InspectImageGetResponse.Builder;
import io.flexio.docker.api.inspectimagegetresponse.json.Status200Reader;
import io.flexio.docker.api.inspectimagegetresponse.json.Status404Reader;
import io.flexio.docker.api.inspectimagegetresponse.json.Status500Reader;
...
public class InspectImageGetResponseReader {
  public InspectImageGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          InspectImageGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    InspectImageGetResponse.Builder builder = InspectImageGetResponse.builder();
  ...
  public InspectImageGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<InspectImageGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new InspectImageGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.InspectImageGetResponse array, current token was %s", parser.currentToken()));
  }
