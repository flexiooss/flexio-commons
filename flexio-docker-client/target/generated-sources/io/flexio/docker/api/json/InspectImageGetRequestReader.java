...
import io.flexio.docker.api.InspectImageGetRequest.Builder;
...
public class InspectImageGetRequestReader {
  private static Set IMAGEID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public InspectImageGetRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          InspectImageGetRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case IMAGEID: {
            Set<JsonToken> expectedTokens = IMAGEID_EXPECTEDTOKENS;
            builder.imageId(this.readValue(parser, jsonParser -> jsonParser.getText(), "imageId", expectedTokens));
            break;
          }
          default: {
  ...
  public InspectImageGetRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<InspectImageGetRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new InspectImageGetRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.InspectImageGetRequest array, current token was %s", parser.currentToken()));
  }
