...
import io.flexio.docker.api.InspectGetRequest.Builder;
...
public class InspectGetRequestReader {
  private static Set CONTAINERID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public InspectGetRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          InspectGetRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public InspectGetRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<InspectGetRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new InspectGetRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.InspectGetRequest array, current token was %s", parser.currentToken()));
  }
