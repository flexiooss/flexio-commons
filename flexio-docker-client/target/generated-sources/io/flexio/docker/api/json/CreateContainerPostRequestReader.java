...
import io.flexio.docker.api.CreateContainerPostRequest.Builder;
import io.flexio.docker.api.types.json.ContainerCreationDataReader;
...
public class CreateContainerPostRequestReader {
  private static Set NAME_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public CreateContainerPostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          CreateContainerPostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case NAME: {
            Set<JsonToken> expectedTokens = NAME_EXPECTEDTOKENS;
            builder.name(this.readValue(parser, jsonParser -> jsonParser.getText(), "name", expectedTokens));
            break;
          }
          case PAYLOAD: {
            parser.nextToken();
            builder.payload(new ContainerCreationDataReader().read(parser));
            break;
          }
          default: {
  ...
  public CreateContainerPostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<CreateContainerPostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new CreateContainerPostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.CreateContainerPostRequest array, current token was %s", parser.currentToken()));
  }
