...
import io.flexio.docker.api.WaitForPostResponse.Builder;
import io.flexio.docker.api.waitforpostresponse.json.Status200Reader;
import io.flexio.docker.api.waitforpostresponse.json.Status404Reader;
import io.flexio.docker.api.waitforpostresponse.json.Status500Reader;
...
public class WaitForPostResponseReader {
  public WaitForPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          WaitForPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    WaitForPostResponse.Builder builder = WaitForPostResponse.builder();
  ...
  public WaitForPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<WaitForPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new WaitForPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.WaitForPostResponse array, current token was %s", parser.currentToken()));
  }
