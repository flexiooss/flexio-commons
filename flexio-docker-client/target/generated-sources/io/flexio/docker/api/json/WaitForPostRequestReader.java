...
import io.flexio.docker.api.WaitForPostRequest.Builder;
...
public class WaitForPostRequestReader {
  private static Set CONDITION_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set CONTAINERID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public WaitForPostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          WaitForPostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case CONDITION: {
            Set<JsonToken> expectedTokens = CONDITION_EXPECTEDTOKENS;
            builder.condition(this.readValue(parser, jsonParser -> jsonParser.getText(), "condition", expectedTokens));
            break;
          }
          case CONTAINERID: {
  ...
  public WaitForPostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<WaitForPostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new WaitForPostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.WaitForPostRequest array, current token was %s", parser.currentToken()));
  }
