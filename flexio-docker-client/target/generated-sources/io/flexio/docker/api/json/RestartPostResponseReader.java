...
import io.flexio.docker.api.RestartPostResponse.Builder;
import io.flexio.docker.api.restartpostresponse.json.Status204Reader;
import io.flexio.docker.api.restartpostresponse.json.Status309Reader;
import io.flexio.docker.api.restartpostresponse.json.Status404Reader;
import io.flexio.docker.api.restartpostresponse.json.Status500Reader;
...
public class RestartPostResponseReader {
  public RestartPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          RestartPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    RestartPostResponse.Builder builder = RestartPostResponse.builder();
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case STATUS204: {
            parser.nextToken();
            builder.status204(new Status204Reader().read(parser));
            break;
          }
          case STATUS309: {
            parser.nextToken();
            builder.status309(new Status309Reader().read(parser));
            break;
          }
          case STATUS404: {
            parser.nextToken();
  ...
  public RestartPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<RestartPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new RestartPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.RestartPostResponse array, current token was %s", parser.currentToken()));
  }
  ...
  enum Token {
    ...
    STATUS204("status204", "status204"),

    STATUS309("status309", "status309"),
