...
import io.flexio.docker.api.ImageListGetResponse.Builder;
import io.flexio.docker.api.imagelistgetresponse.json.Status200Reader;
import io.flexio.docker.api.imagelistgetresponse.json.Status500Reader;
...
public class ImageListGetResponseReader {
  public ImageListGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ImageListGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public ImageListGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ImageListGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ImageListGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.ImageListGetResponse array, current token was %s", parser.currentToken()));
  }
