...
import io.flexio.docker.api.VersionGetRequest.Builder;
...
public class VersionGetRequestReader {
  public VersionGetRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          VersionGetRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public VersionGetRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<VersionGetRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new VersionGetRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.VersionGetRequest array, current token was %s", parser.currentToken()));
  }
