...
import io.flexio.docker.api.RestartPostRequest.Builder;
...
public class RestartPostRequestReader {
  private static Set T_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_NUMBER_INT));

  private static Set CONTAINERID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public RestartPostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          RestartPostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case T: {
            Set<JsonToken> expectedTokens = T_EXPECTEDTOKENS;
            builder.t(this.readValue(parser, jsonParser -> jsonParser.getLongValue(), "t", expectedTokens));
            break;
          }
          case CONTAINERID: {
  ...
  public RestartPostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<RestartPostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new RestartPostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.RestartPostRequest array, current token was %s", parser.currentToken()));
  }
