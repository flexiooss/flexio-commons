...
import io.flexio.docker.api.VersionGetResponse.Builder;
import io.flexio.docker.api.versiongetresponse.json.Status200Reader;
...
public class VersionGetResponseReader {
  public VersionGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          VersionGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    VersionGetResponse.Builder builder = VersionGetResponse.builder();
  ...
  public VersionGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<VersionGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new VersionGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.VersionGetResponse array, current token was %s", parser.currentToken()));
  }
