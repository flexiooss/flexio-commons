...
import io.flexio.docker.api.StopPostRequest.Builder;
...
public class StopPostRequestReader {
  ...
  public StopPostRequest read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          StopPostRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public StopPostRequest[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<StopPostRequest> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new StopPostRequest[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.StopPostRequest array, current token was %s", parser.currentToken()));
  }
