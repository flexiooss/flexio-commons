...
import io.flexio.docker.api.containerlistgetresponse.Status200.Builder;
...
public class Status200Reader {
  public Status200 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status200.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case PAYLOAD: {
            ContainerInListReader reader = new ContainerInListReader();
  ...
  public Status200[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status200> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status200[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.containerlistgetresponse.Status200 array, current token was %s", parser.currentToken()));
  }
