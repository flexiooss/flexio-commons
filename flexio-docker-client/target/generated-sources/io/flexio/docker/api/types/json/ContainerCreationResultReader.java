...
import io.flexio.docker.api.types.ContainerCreationResult.Builder;
...
public class ContainerCreationResultReader {
  private static Set ID_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set WARNING_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public ContainerCreationResult read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerCreationResult.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          ...
          case WARNING: {
            Set<JsonToken> expectedTokens = WARNING_EXPECTEDTOKENS;
            builder.warning(this.readValue(parser, jsonParser -> jsonParser.getText(), "warning", expectedTokens));
            break;
          }
          default: {
  ...
  public ContainerCreationResult[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerCreationResult> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerCreationResult[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.types.ContainerCreationResult array, current token was %s", parser.currentToken()));
  }
