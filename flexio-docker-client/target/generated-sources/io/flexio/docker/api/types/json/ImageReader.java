...
import io.flexio.docker.api.types.Image.Builder;
...
public class ImageReader {
  ...
  public Image read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Image.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          ...
          case REPOTAGS: {
            builder.repoTags(this.readListValue(parser, jsonParser -> jsonParser.getText(), "repoTags"));
            break;
          }
          default: {
  ...
  public Image[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Image> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Image[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.types.Image array, current token was %s", parser.currentToken()));
  }
