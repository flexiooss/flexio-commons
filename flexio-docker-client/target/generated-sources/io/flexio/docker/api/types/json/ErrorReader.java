...
import io.flexio.docker.api.types.Error.Builder;
...
public class ErrorReader {
  ...
  public Error read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Error.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public Error[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Error> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Error[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.types.Error array, current token was %s", parser.currentToken()));
  }
