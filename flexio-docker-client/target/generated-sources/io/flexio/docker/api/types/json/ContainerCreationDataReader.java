...
import io.flexio.docker.api.types.ContainerCreationData.Builder;
...
public class ContainerCreationDataReader {
  private static Set IMAGE_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public ContainerCreationData read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerCreationData.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          ...
          case CMD: {
            builder.cmd(this.readListValue(parser, jsonParser -> jsonParser.getText(), "cmd"));
            break;
          }
          default: {
  ...
  public ContainerCreationData[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerCreationData> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerCreationData[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.types.ContainerCreationData array, current token was %s", parser.currentToken()));
  }
