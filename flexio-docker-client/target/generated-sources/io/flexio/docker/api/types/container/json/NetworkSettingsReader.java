...
import io.flexio.docker.api.types.container.NetworkSettings.Builder;
...
public class NetworkSettingsReader {
  private static Set IPADDRESS_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public NetworkSettings read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          NetworkSettings.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case IPADDRESS: {
            Set<JsonToken> expectedTokens = IPADDRESS_EXPECTEDTOKENS;
            builder.iPAddress(this.readValue(parser, jsonParser -> jsonParser.getText(), "iPAddress", expectedTokens));
            break;
          }
          default: {
  ...
  public NetworkSettings[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<NetworkSettings> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new NetworkSettings[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.types.container.NetworkSettings array, current token was %s", parser.currentToken()));
  }
