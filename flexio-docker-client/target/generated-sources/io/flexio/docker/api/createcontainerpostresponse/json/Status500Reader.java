...
import io.flexio.docker.api.createcontainerpostresponse.Status500.Builder;
...
public class Status500Reader {
  public Status500 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status500.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public Status500[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status500> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status500[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.createcontainerpostresponse.Status500 array, current token was %s", parser.currentToken()));
  }
