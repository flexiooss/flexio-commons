...
import io.flexio.docker.api.createcontainerpostresponse.Status201.Builder;
...
public class Status201Reader {
  public Status201 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status201.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case PAYLOAD: {
            parser.nextToken();
            builder.payload(new ContainerCreationResultReader().read(parser));
            break;
          }
          default: {
  ...
  public Status201[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status201> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status201[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.createcontainerpostresponse.Status201 array, current token was %s", parser.currentToken()));
  }
