...
import io.flexio.docker.api.createcontainerpostresponse.Status409.Builder;
...
public class Status409Reader {
  public Status409 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status409.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public Status409[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status409> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status409[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.api.createcontainerpostresponse.Status409 array, current token was %s", parser.currentToken()));
  }
