...
import io.flexio.docker.descriptors.ContainerCreationLog.Builder;
...
public class ContainerCreationLogReader {
  ...
  public ContainerCreationLog read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerCreationLog.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ContainerCreationLog.Builder builder = ContainerCreationLog.builder();
      ...
      if(token != null) {
        switch (token) {
          ...
          case ACTION: {
            ...
            try {
              builder.action(this.readValue(parser, jsonParser -> ContainerCreationLog.Action.valueOf(jsonParser.getText()), "action", expectedTokens));
  ...
  public ContainerCreationLog[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerCreationLog> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerCreationLog[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.descriptors.ContainerCreationLog array, current token was %s", parser.currentToken()));
  }
