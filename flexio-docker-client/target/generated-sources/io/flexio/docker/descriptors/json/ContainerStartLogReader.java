...
import io.flexio.docker.descriptors.ContainerStartLog.Builder;
...
public class ContainerStartLogReader {
  ...
  public ContainerStartLog read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerStartLog.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ContainerStartLog.Builder builder = ContainerStartLog.builder();
      ...
      if(token != null) {
        switch (token) {
          ...
          case ACTION: {
            ...
            try {
              builder.action(this.readValue(parser, jsonParser -> ContainerStartLog.Action.valueOf(jsonParser.getText()), "action", expectedTokens));
  ...
  public ContainerStartLog[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerStartLog> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerStartLog[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.descriptors.ContainerStartLog array, current token was %s", parser.currentToken()));
  }
