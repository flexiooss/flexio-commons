...
import io.flexio.docker.descriptors.ContainerStopLog.Builder;
...
public class ContainerStopLogReader {
  ...
  public ContainerStopLog read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          ContainerStopLog.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ContainerStopLog.Builder builder = ContainerStopLog.builder();
      ...
      if(token != null) {
        switch (token) {
          ...
          case ACTION: {
            ...
            try {
              builder.action(this.readValue(parser, jsonParser -> ContainerStopLog.Action.valueOf(jsonParser.getText()), "action", expectedTokens));
  ...
  public ContainerStopLog[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<ContainerStopLog> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new ContainerStopLog[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.docker.descriptors.ContainerStopLog array, current token was %s", parser.currentToken()));
  }
