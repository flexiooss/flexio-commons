...
public class ETaggedChangeTest {
    ...
    private final TestApiHandlers handlers = new TestApiHandlers.Builder()
            .resourcePutHandler(
                ETaggedChange.forHandler(this.put, ResourcePutResponse.class)
                    .idFromRequest(request -> COMPLETE_UNDERLYING_RESPONSE.status200().xEntityId())
                    .defaultCacheControl("test-cache-control")
                    .using(this.etags)
            )
            .build();
    ...
    @Test
    public void givenNoETagRequest_andDelegateReturns200__whenDelegateDoesntSetCacheControl__thenDefaltCacheControlSet() throws Exception {
        this.put.nextResponse(request -> COMPLETE_UNDERLYING_RESPONSE.withStatus200(COMPLETE_UNDERLYING_RESPONSE.status200().withCacheControl(null)));
