...
import io.flexio.commons.graylog.api.AbsoluteGetResponse.Builder;
import io.flexio.commons.graylog.api.absolutegetresponse.json.Status200Reader;
import io.flexio.commons.graylog.api.absolutegetresponse.json.Status400Reader;
...
public class AbsoluteGetResponseReader {
  public AbsoluteGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          AbsoluteGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    AbsoluteGetResponse.Builder builder = AbsoluteGetResponse.builder();
  ...
  public AbsoluteGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<AbsoluteGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new AbsoluteGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.graylog.api.AbsoluteGetResponse array, current token was %s", parser.currentToken()));
  }
