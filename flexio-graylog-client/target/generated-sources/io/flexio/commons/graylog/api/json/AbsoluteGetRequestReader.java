...
import io.flexio.commons.graylog.api.AbsoluteGetRequest.Builder;
...
public class AbsoluteGetRequestReader {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          AbsoluteGetRequest.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
  ...
  public AbsoluteGetRequest[] readArray(JsonParser parser) throws IOException {
    ...
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<AbsoluteGetRequest> listValue = new LinkedList<>();
