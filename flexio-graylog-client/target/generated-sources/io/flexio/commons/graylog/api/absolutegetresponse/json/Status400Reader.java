...
import io.flexio.commons.graylog.api.absolutegetresponse.Status400.Builder;
...
public class Status400Reader {
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case PAYLOAD: {
            parser.nextToken();
            builder.payload(new ObjectValueReader().read(parser));
            break;
          }
          default: {
  ...
  public Status400[] readArray(JsonParser parser) throws IOException {
    ...
    throw new IOException(String.format("failed reading io.flexio.commons.graylog.api.absolutegetresponse.Status400 array, current token was %s", parser.currentToken()));
  }
