...
import io.flexio.commons.graylog.api.RelativeGetResponse.Builder;
import io.flexio.commons.graylog.api.relativegetresponse.json.Status200Reader;
import io.flexio.commons.graylog.api.relativegetresponse.json.Status400Reader;
...
public class RelativeGetResponseReader {
  public RelativeGetResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          RelativeGetResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    RelativeGetResponse.Builder builder = RelativeGetResponse.builder();
  ...
  public RelativeGetResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<RelativeGetResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new RelativeGetResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.graylog.api.RelativeGetResponse array, current token was %s", parser.currentToken()));
  }
