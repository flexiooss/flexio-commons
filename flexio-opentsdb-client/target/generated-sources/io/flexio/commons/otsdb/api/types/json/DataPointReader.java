...
import io.flexio.commons.otsdb.api.types.DataPoint.Builder;
...
public class DataPointReader {
  private static Set METRIC_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  private static Set TIMESTAMP_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_NUMBER_INT));

  private static Set VALUE_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_NUMBER_FLOAT));

  public DataPoint read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          DataPoint.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case METRIC: {
            Set<JsonToken> expectedTokens = METRIC_EXPECTEDTOKENS;
            builder.metric(this.readValue(parser, jsonParser -> jsonParser.getText(), "metric", expectedTokens));
            break;
          }
          case TIMESTAMP: {
            Set<JsonToken> expectedTokens = TIMESTAMP_EXPECTEDTOKENS;
            builder.timestamp(this.readValue(parser, jsonParser -> jsonParser.getLongValue(), "timestamp", expectedTokens));
            break;
          }
          case VALUE: {
            Set<JsonToken> expectedTokens = VALUE_EXPECTEDTOKENS;
            builder.value(this.readValue(parser, jsonParser -> jsonParser.getDoubleValue(), "value", expectedTokens));
            break;
          }
          case TAGS: {
            parser.nextToken();
            builder.tags(new ObjectValueReader().read(parser));
            break;
          }
          default: {
  ...
  public DataPoint[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<DataPoint> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new DataPoint[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.otsdb.api.types.DataPoint array, current token was %s", parser.currentToken()));
  }
