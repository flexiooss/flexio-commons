...
import io.flexio.commons.otsdb.api.types.DataPointError.Builder;
...
public class DataPointErrorReader {
  private static Set ERROR_EXPECTEDTOKENS = new HashSet(Arrays.asList(JsonToken.VALUE_STRING));

  public DataPointError read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          DataPointError.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case DATAPOINT: {
            parser.nextToken();
            builder.datapoint(new DataPointReader().read(parser));
            break;
          }
          case ERROR: {
            Set<JsonToken> expectedTokens = ERROR_EXPECTEDTOKENS;
            builder.error(this.readValue(parser, jsonParser -> jsonParser.getText(), "error", expectedTokens));
            break;
          }
          default: {
  ...
  public DataPointError[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<DataPointError> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new DataPointError[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.otsdb.api.types.DataPointError array, current token was %s", parser.currentToken()));
  }
