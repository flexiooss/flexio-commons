...
import io.flexio.commons.otsdb.api.storedatapointspostresponse.Status400.Builder;
...
public class Status400Reader {
  public Status400 read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          Status400.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    ...
    while (parser.nextToken() != JsonToken.END_OBJECT) {
      ...
      if(token != null) {
        switch (token) {
          case PAYLOAD: {
            parser.nextToken();
            builder.payload(new StorageResponseReader().read(parser));
            break;
          }
          default: {
  ...
  public Status400[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<Status400> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new Status400[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.otsdb.api.storedatapointspostresponse.Status400 array, current token was %s", parser.currentToken()));
  }
