...
import io.flexio.commons.otsdb.api.StoreDataPointsPostResponse.Builder;
import io.flexio.commons.otsdb.api.storedatapointspostresponse.json.Status204Reader;
import io.flexio.commons.otsdb.api.storedatapointspostresponse.json.Status400Reader;
...
public class StoreDataPointsPostResponseReader {
  public StoreDataPointsPostResponse read(JsonParser parser) throws IOException {
    ...
    if(parser.currentToken() != JsonToken.START_OBJECT) {
      throw new IOException(
                  String.format("reading a %s object, was expecting %s, but was %s",
                          StoreDataPointsPostResponse.class.getName(), JsonToken.START_OBJECT, parser.currentToken()
                  )
          );
    }
    StoreDataPointsPostResponse.Builder builder = StoreDataPointsPostResponse.builder();
  ...
  public StoreDataPointsPostResponse[] readArray(JsonParser parser) throws IOException {
    parser.nextToken();
    if (parser.currentToken() == JsonToken.VALUE_NULL) return null;
    if (parser.currentToken() == JsonToken.START_ARRAY) {
      LinkedList<StoreDataPointsPostResponse> listValue = new LinkedList<>();
      ...
      return listValue.toArray(new StoreDataPointsPostResponse[listValue.size()]);
    }
    throw new IOException(String.format("failed reading io.flexio.commons.otsdb.api.StoreDataPointsPostResponse array, current token was %s", parser.currentToken()));
  }
